...
{ const p = await prisma.product.findUnique({ where: { id: req.params.id }, include: { category: true } }); if (!p) return res.status(404).json({ success: false, message: 'محصول یافت نشد.' }); res.json({ success: true, product: toProduct(p) }); }
